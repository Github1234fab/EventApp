  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const cats = Array.from(
          new Set(eventsData.map((e) => (e?.catégorie || "").trim()).filter(Boolean))
        ).sort((a, b) => a.localeCompare(b));
        setCategories(cats);

        const lieuxUnique = Array.from(
          new Set(eventsData.map((e) => (e?.lieu || "").trim()).filter(Boolean))
        ).sort((a, b) => a.localeCompare(b));
        setLieux(lieuxUnique);
      } catch (error) {
        console.error("Erreur Firestore :", error);
  ...
  useEffect(() => {
    ...
    if (selectedDateFilter !== "Toutes") {
      ...
      filtered = filtered.filter((e) => {
        ...
        if (!eventDate.isValid()) return false;
        switch (selectedDateFilter) {
          case "Aujourd'hui": return eventDate.isSame(now, "day");
          case "Cette semaine": return eventDate.isSame(now, "week");
          case "Ce mois": return eventDate.isSame(now, "month");
          case "Cette année": return eventDate.isSame(now, "year");
          default: return true;
        }
      });
    }
      ...
      <View style={styles.topBar}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.actionsRow}>
          ...
          <TouchableOpacity style={styles.compactButton} onPress={() => navigation.navigate("MyAds")}>
            <Text style={styles.buttonText}>📁 Annonces</Text>
          </TouchableOpacity>
      ...
      {showFilters && (
        <View style={styles.filters}>
          ...
          <Picker selectedValue={selectedLieu} onValueChange={(itemValue) => setSelectedLieu(itemValue)} style={styles.picker}>
            <Picker.Item label="Tous lieux" value="Tous" />
            {lieux.map((lieu, index) => (<Picker.Item key={index} label={lieu} value={lieu} />))}
          </Picker>
          ...
          <TouchableOpacity style={styles.resetButton} onPress={() => { setSelectedCategory("Toutes"); setSelectedLieu("Tous"); setSelectedDateFilter("Toutes"); }}>
...
const styles = StyleSheet.create({
  ...
  topBar: { backgroundColor: "#fff", paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#e0e0e0", elevation: 2, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  actionsRow: { flexDirection: "row", paddingHorizontal: 12, gap: 8 },
  compactButton: { backgroundColor: "#007AFF", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, justifyContent: "center", alignItems: "center" },
  logoutButton: { backgroundColor: "#FF3B30" },
  buttonText: { color: "#fff", fontSize: 13, fontWeight: "600" },
  headerContainer: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#e0e0e0" },
  header: { fontSize: 18, fontWeight: "bold" },
  filterToggle: { backgroundColor: "#34C759", paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16 },
  filterToggleText: { color: "#fff", fontSize: 13, fontWeight: "600" },
  filters: { backgroundColor: "#fff", paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "#e0e0e0" },
  picker: { marginVertical: 4, backgroundColor: "#f5f5f5", borderRadius: 8 },
  resetButton: { backgroundColor: "#FF9500", paddingVertical: 10, borderRadius: 8, marginTop: 8, alignItems: "center" },
