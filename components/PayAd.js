...
export default function PayAd({ route, navigation }) {
  ...
  const startCheckout = useCallback(async () => {
    try {
      ...
      // ✅ Deep link de retour AVEC l'ID
      const returnUrl = Linking.createURL("payment-return", {
        queryParams: { submissionId },
      });
      console.log("[PayAd] returnUrl généré:", returnUrl);
      ...
      // ✅ IMPORTANT : utiliser openAuthSessionAsync (et PAS openBrowserAsync)
      const result = await WebBrowser.openAuthSessionAsync(url, returnUrl);

      // ⬇⬇⬇ FALLBACK si le deep link n'a pas été déclenché (Expo Go tué / app banque etc.)
      if (result.type !== "success") {
        console.log("[PayAd] Pas de redirection deep link, fallback vers PaymentReturn");
        navigation.navigate("PaymentReturn", { submissionId });
      }
      // ⬆⬆⬆
