  ...
  return cors(req, res, async () => {
    try {
      ...
      if (!submissionId || typeof submissionId !== "string") {
        return res.status(400).json({ error: "submissionId (string) requis" });
      }

      if (typeof price !== "number" || price < 50) {
        return res.status(400).json({ error: "price doit être >= 50 centimes" });
      }

      // Vérifier que la soumission existe et n'est pas déjà payée
      const db = admin.firestore();
      const docRef = db.collection("Submissions").doc(submissionId);
      const docSnap = await docRef.get();

      if (!docSnap.exists) {
        console.error("[createCheckoutSession] Submission introuvable:", submissionId);
        return res.status(404).json({ error: "Annonce introuvable" });
      }

      const data = docSnap.data();
      if (data.paid === true) {
        console.log("[createCheckoutSession] Déjà payée:", submissionId);
        return res.status(400).json({ error: "Annonce déjà payée" });
      }

      // URLs de retour (à adapter selon votre deep link)
      const success_url = returnUrl || "https://example.com/success";
      const cancel_url = returnUrl || "https://example.com/cancel";
        ...
        line_items: [
          {
            price_data: {
              currency: "eur",
              product_data: {
                name: `Annonce: ${data.titre || "Sans titre"}`,
                description: `Publication d'annonce - ${data.lieu || ""}`,
              },
              unit_amount: price,
            },
            quantity: 1,
      ...
      console.error("[createCheckoutSession] ERROR:", e.message, e.stack);
...
exports.stripeWebhook = onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!sig) {
      console.error("[stripeWebhook] Pas de signature Stripe");
      return res.status(400).send("No signature");
    }

    if (!endpointSecret) {
      console.error("[stripeWebhook] STRIPE_WEBHOOK_SECRET non configuré");
    ...
    if (!submissionId) {
      console.error("[stripeWebhook] Aucun submissionId dans la session");
      return res.status(200).send("No submissionId");
    }

    // Vérifier que le paiement est bien réussi
    if (session.payment_status !== "paid") {
      console.warn("[stripeWebhook] Paiement non complété:", session.payment_status);
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (!snap.exists) {
        console.error("[stripeWebhook] Submission introuvable:", submissionId);
      ...
      // Anti-doublon
      if (data.paid === true) {
        console.log("[stripeWebhook] Déjà marqué comme payé:", submissionId);
        return;
      }
      ...
      console.log("[stripeWebhook] ✅ Paiement confirmé pour:", submissionId);
    });

    return res.status(200).send("ok");
  } catch (e) {
    console.error("[stripeWebhook] ERROR:", e.message, e.stack);
    return res.status(500).send("Server error");
  }
});
